...
addBtn.addEventListener("click", function () {
  ...
  updateProgress(); // Update progress whenever a task is added
});
...
function ticketAddertoUI(arr) {
  ...
  arr.forEach(function (taskObj) {
    ...
    delIcon.addEventListener("click", function () {
      ...
      localStorage.setItem("Tasks", JSON.stringify(tasksArray));
      updateProgress(); // Update progress whenever a task is deleted
    });

    cont.appendChild(taskEle);
  });
  updateProgress(); // Ensure progress is updated when the UI is updated
}

function updateProgress() {
  const tasks = document.querySelectorAll('.cont .task').length;
  const progressBar = document.getElementById('progress-bar');
  const maxTasks = 10; // Adjust max tasks as needed

  const progressValue = Math.min((tasks / maxTasks) * 100, 100);
